...
int AAS_Reachability_Ladder(int area1num, int area2num)
{
	...
	if (ladderface1 && ladderface2)
	{
		...
		//
		ladderface1vertical = abs((int)DotProduct(plane1->normal, up)) < 0.1;
		ladderface2vertical = abs((int)DotProduct(plane2->normal, up)) < 0.1;
		...
		if (ladderface1vertical && ladderface2vertical
					...
					&& abs((int)DotProduct(sharededgevec, up)) < 0.7)
		{
			//create a new reachability link
			lreach = AAS_AllocReachability();
			if (!lreach) return qfalse;
		...
		if (ladderface1vertical)
		{
			...
			for (i = 0; i < area2->numfaces; i++)
			{
				...
				{
					plane2 = &aasworld.planes[face2->planenum];
					if (abs((int)DotProduct(plane2->normal, up)) < 0.1) break;
				} //end if
			} //end for
