...
void *RE_RegisterModels_Malloc(int iSize, const char *psModelFileName, qboolean *pqbAlreadyFound, memtag_t eTag)
{
	...
	if (ModelBin.pModelDiskImage == NULL)
	{
		...
		{
			Com_Error( ERR_FATAL, "RE_RegisterModels_Malloc(): Failed to alloc %d bytes for \"%s\"!", iSize, sModelName);
		}
...
void *RE_RegisterServerModels_Malloc(int iSize, const char *psModelFileName, qboolean *pqbAlreadyFound, memtag_t eTag)
{
	...
	if (ModelBin.pModelDiskImage == NULL)
	{
		...
		if (!pMalloc)	// not needed anymore, but wtf?
		{
			Com_Error( ERR_FATAL, "RE_RegisterServerModels_Malloc(): Failed to alloc %d bytes for \"%s\"!", iSize, sModelName);
		}
...
model_t *R_AllocModel( void ) {
	...
	mod = (struct model_s *)ri.Hunk_Alloc( sizeof( *tr.models[tr.numModels] ), h_low );
...
void RE_InsertModelIntoHash(const char *name, model_t *mod)
{
	...
	mh = (modelHash_t*)ri.Hunk_Alloc( sizeof( modelHash_t ), h_low );
...
void GenericInsertModelIntoHash(const char *name, model_t *mod)
{
	int			hash;
	modelHash_t	*mh;
...
** GenericAllocModel
*/
model_t *GenericAllocModel( void ) {
	model_t		*mod;
...
*/
	...
	GenericInsertModelIntoHash(name, mod);
	return mod->index;
/*
Ghoul2 Insert End
*/
