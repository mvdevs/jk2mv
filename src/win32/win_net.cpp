...
static qboolean NET_GetCvars( void ) {
	...
	net_noudp = Cvar_Get("net_noudp", "0", CVAR_LATCH | CVAR_ARCHIVE | CVAR_GLOBAL);

	if( net_noipx && net_noipx->modified ) {
		modified = qtrue;
	}
	net_noipx = Cvar_Get("net_noipx", "1", CVAR_LATCH | CVAR_ARCHIVE | CVAR_GLOBAL);


	if( net_forcenonlocal && net_forcenonlocal->modified ) {
		modified = qtrue;
	}
	net_forcenonlocal = Cvar_Get("net_forcenonlocal", "0", CVAR_LATCH | CVAR_ARCHIVE | CVAR_GLOBAL);


	if( net_socksEnabled && net_socksEnabled->modified ) {
		modified = qtrue;
	}
	net_socksEnabled = Cvar_Get("net_socksEnabled", "0", CVAR_LATCH | CVAR_ARCHIVE | CVAR_GLOBAL);

	if( net_socksServer && net_socksServer->modified ) {
		modified = qtrue;
	}
	net_socksServer = Cvar_Get("net_socksServer", "", CVAR_LATCH | CVAR_ARCHIVE | CVAR_GLOBAL);

	if( net_socksPort && net_socksPort->modified ) {
		modified = qtrue;
	}
	net_socksPort = Cvar_Get("net_socksPort", "1080", CVAR_LATCH | CVAR_ARCHIVE | CVAR_GLOBAL);

	if( net_socksUsername && net_socksUsername->modified ) {
		modified = qtrue;
	}
	net_socksUsername = Cvar_Get("net_socksUsername", "", CVAR_LATCH | CVAR_ARCHIVE | CVAR_GLOBAL);

	if( net_socksPassword && net_socksPassword->modified ) {
		modified = qtrue;
	}
	net_socksPassword = Cvar_Get("net_socksPassword", "", CVAR_LATCH | CVAR_ARCHIVE | CVAR_GLOBAL);
