...
qboolean IN_InitDIMouse( void ) {
	...
	if (!pDirectInputCreate) {
		pDirectInputCreate = (long (__stdcall *)(struct HINSTANCE__ *,unsigned long,struct IDirectInput ** ,struct IUnknown *))
...
void IN_Init( void ) {
	...
	in_midi = Cvar_Get("in_midi", "0", CVAR_ARCHIVE | CVAR_GLOBAL);
	in_midiport = Cvar_Get("in_midiport", "1", CVAR_ARCHIVE | CVAR_GLOBAL);
	in_midichannel = Cvar_Get("in_midichannel", "1", CVAR_ARCHIVE | CVAR_GLOBAL);
	in_mididevice = Cvar_Get("in_mididevice", "0", CVAR_ARCHIVE | CVAR_GLOBAL);

	Cmd_AddCommand( "midiinfo", MidiInfo_f );

	// mouse variables
	in_mouse = Cvar_Get("in_mouse", "-1", CVAR_ARCHIVE | CVAR_LATCH | CVAR_GLOBAL);

	// joystick variables
	in_joystick = Cvar_Get("in_joystick", "0", CVAR_ARCHIVE | CVAR_LATCH | CVAR_GLOBAL);
	in_joyBallScale = Cvar_Get("in_joyBallScale", "0.02", CVAR_ARCHIVE | CVAR_GLOBAL);
	in_debugJoystick		= Cvar_Get ("in_debugjoystick",			"0",		CVAR_TEMP);

	joy_threshold = Cvar_Get("joy_threshold", "0.15", CVAR_ARCHIVE | CVAR_GLOBAL);
