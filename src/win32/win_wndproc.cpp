...
static void WIN_DisableAltTab( void )
{
	...
	else
	{
		...
		SystemParametersInfoA( SPI_SCREENSAVERRUNNING, 1, &old, 0 );
	}
...
static void WIN_EnableAltTab( void )
{
	if ( s_alttab_disabled )
	{
		...
		else
		{
			...
			SystemParametersInfoA( SPI_SCREENSAVERRUNNING, 0, &old, 0 );
		}
    ...
    LPARAM  lParam)
{
	...
	case WM_CREATE:
		...
		vid_xpos = Cvar_Get("vid_xpos", "3", CVAR_ARCHIVE | CVAR_GLOBAL);
		vid_ypos = Cvar_Get("vid_ypos", "22", CVAR_ARCHIVE | CVAR_GLOBAL);
		r_fullscreen = Cvar_Get("r_fullscreen", "1", CVAR_ARCHIVE | CVAR_LATCH | CVAR_GLOBAL);
