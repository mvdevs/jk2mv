...
#ifdef USE_JK2
	mdxa = mdxa = mod->data.gla = (mdxaHeader_t*) //Hunk_Alloc( size );
...
qboolean ServerLoadMDXM( model_t *mod, void *buffer, const char *mod_name, qboolean &bAlreadyCached ) {
	...
	qboolean bAlreadyFound = qfalse;
#ifdef USE_JK2
	mdxm = (mdxmHeader_t*)RE_RegisterServerModels_Malloc(size, mod_name, &bAlreadyFound, TAG_MODEL_GLM);
#else
...
static qboolean R_LoadMD3 ( model_t *mod, int lod, void *buffer, const char *mod_name, qboolean &bAlreadyCached ) {
	...
	qboolean bAlreadyFound = qfalse;
	md3Model = (md3Header_t *)RE_RegisterModels_Malloc(size, mod_name, &bAlreadyFound, TAG_MODEL_MD3);
