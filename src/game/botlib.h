...
typedef struct botlib_import_s
{
	...
	int			(*FS_FOpenFileHash)( const char *qpath, fileHandle_t *file, fsMode_t mode, unsigned long *hash );
...
typedef struct ai_export_s
{
	...
	void	(*BotMatchVariable)(const struct bot_match_s *match, int variable, char *buf, int size);
	...
	void	(*BotPushGoal)(int goalstate, const struct bot_goal_s *goal);
	...
	int		(*BotChooseNBGItem)(int goalstate, const vec3_t origin, const int *inventory, int travelflags,
								const struct bot_goal_s *ltg, float maxtime);
	int		(*BotTouchingGoal)(const vec3_t origin, const struct bot_goal_s *goal);
	int		(*BotItemGoalInVisButNotVisible)(int viewer, const vec3_t eye, const vec3_t viewangles, const struct bot_goal_s *goal);
	...
	void	(*BotMoveToGoal)(struct bot_moveresult_s *result, int movestate, const struct bot_goal_s *goal, int travelflags);
	...
	int		(*BotMovementViewTarget)(int movestate, const struct bot_goal_s *goal, int travelflags, float lookahead, vec3_t target);
	int		(*BotPredictVisiblePosition)(const vec3_t origin, int areanum, const struct bot_goal_s *goal, int travelflags, vec3_t target);
	int		(*BotAllocMoveState)(void);
	void	(*BotFreeMoveState)(int handle);
	void	(*BotInitMoveState)(int handle, const struct bot_initmove_s *initmove);
...
typedef struct botlib_export_s
{
	...
	//entity updates
	int (*BotLibUpdateEntity)(int ent, const bot_entitystate_t *state);
	//just for testing
