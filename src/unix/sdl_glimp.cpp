...
static rserr_t GLimp_SetMode(int mode, qboolean fullscreen, qboolean noborder)
{
	...
	if( SDL_GetDesktopDisplayMode( display, &desktopMode ) == 0 || desktopMode.h > 0 )
	{
	...
	for (i = 0; i < 16; i++)
	{
		...
		if( ( screen = SDL_CreateWindow( "JK2MV", x, y,
		...
		SDL_SetWindowTitle( screen, "JK2MV" );
...
{
	const char *ptr = strcasestr( glConfig.extensions_string, ext );
	if (ptr == NULL)
		return false;
	ptr += strlen(ext);
...
void 		GLimp_Init( void )
{
	...
	if(GLimp_StartDriverAndSetMode(r_mode->integer, (qboolean)r_fullscreen->integer, qfalse))
		goto success;
