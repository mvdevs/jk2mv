...
//  pmin & pmax are used to return the parse values
//
// return:
//	success of parse operation.
//------------------------------------------------------
#pragma auto_inline ( off )//don't even think about inlining this pos - because it generate warnings all over the place.
bool CPrimitiveTemplate::ParseFloat( const char *val, float *pMin, float *pMax )
{
	...
	int v = sscanf( val, "%f %f", pMin, pMax );
	...
	{ // only one field entered, this is ok, but we should copy pMin into pMax
		*pMax = *pMin;
	}
