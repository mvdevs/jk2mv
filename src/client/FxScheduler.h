...
#ifdef EFFECTSED
	// For the editor, which needs to iterate through the list and/or clear it.
	vector<int> &GetHandleArray()	{ return mMediaList; }
	const vector<int> &GetHandleArray() const	{ return mMediaList; }

private:
	vector<string> mMediaNames;
public:
	void PrepareReloadAssets(enum EMediaTypes mediaType);
	void ReloadAssets(enum EMediaTypes mediaType);
#endif
};
...
#ifdef EFFECTSED
	friend void UnparseEffect(int id, std::string &str);

	bool	mbStopScheduled;
#endif
...
public:
	...
	int		NewEffect();	// create a new effect template containing no primitives
	void	RegisterEffectWithEditor( int id );
	void	NewPrimitive( int id, EPrimType type );
	void	DeletePrimitive( int id, CPrimitiveTemplate *prim );

	void	ScheduleStop()		{ mbStopScheduled = true; }
	bool	IsStopScheduled()	{ return mbStopScheduled; }

	void	PrepareReloadAssets(int idToPreserve);
	void	ReloadAssets(int idToPreserve);
#endif
...
#ifndef EFFECTSED
#ifndef CHC // SOF2 only
	void	PlayEffect( int id, CFxBoltInterface *obj );
#endif
#endif
