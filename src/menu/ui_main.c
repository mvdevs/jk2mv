...
void _UI_Refresh( int realtime )
{
	...
	UI_SetColor( NULL );
	if (Menu_Count() > 0) {
		UI_DrawHandlePic( uiInfo.uiDC.cursorx, uiInfo.uiDC.cursory, 48 * ((float)(SCREEN_WIDTH * uiInfo.uiDC.glconfig.vidHeight) / (float)(SCREEN_HEIGHT * uiInfo.uiDC.glconfig.vidWidth)), 48, uiInfo.uiDC.Assets.cursor);
	}
...
static void UI_BuildPlayerList() {
	...
	for( n = 0; n < count; n++ ) {
		...
		if (info[0]) {
			...
			Q_CleanStr( uiInfo.playerNames[uiInfo.playerCount], qfalse );
			uiInfo.playerIndexes[uiInfo.playerCount] = n;
			uiInfo.playerCount++;
			team2 = atoi(Info_ValueForKey(info, "t"));
			if (team2 == team && n != uiInfo.playerNumber) {
				Q_strncpyz( uiInfo.teamNames[uiInfo.myTeamCount], Info_ValueForKey( info, "n" ), MAX_NAME_LENGTH );
				Q_CleanStr( uiInfo.teamNames[uiInfo.myTeamCount], qfalse );
...
static void UI_BuildFindPlayerList(qboolean force) {
	...
	if (!force) {
		...
		Q_CleanStr(uiInfo.findPlayerName, qfalse);
