...
//#define MV_UI_PATCHMENU // JK2MV: Avoid editing the menu-files
...
qboolean PC_String_Parse(int handle, const char **out)
{
	...
	if (strlen(token.string) && !Q_stricmp(token.string, "@MENUS0_SET_FILTER_FOR_SPECIFIC"))
		Q_strncpyz(token.string, "@MV_GAME_VERSION_BROWSER_INFO", sizeof(token.string));
#endif
...
#ifdef MV_UI_PATCHMENU // JK2MV: Avoid editing the menu-files -> joinserver.menu
	if ( item && item->window.name && (!Q_stricmp(item->window.name, "datarate_button") || !Q_stricmp(item->window.name, "datarate")) ) return qtrue;
#endif
...
qboolean Menu_Parse(int handle, menuDef_t *menu) {
	...
	while ( 1 ) {
		...
		if (*token.string == '}') {
#ifdef MV_UI_PATCHMENU // JK2MV: Avoid editing the menu-files -> createserver.menu
			if ( menu && menu->window.name && !Q_stricmp(menu->window.name, "createserver") )
			{
				...
				while ( 1 )
				{
					...
					if ( !key->func((itemDef_t*)menu, handle_addon) ) {
